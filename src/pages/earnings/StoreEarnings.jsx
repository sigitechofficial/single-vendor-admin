...
export default function StoreEarnings() {
  ...
  const getData = async () => {
    let res = await PostAPI("admin/store_earnings", {
      businessType: "3",
      zoneId: filterData?.zone?.value,
      storeId: filterData?.store?.value,
  ...
  const fetchStores = async (zoneId = null) => {
    const res = await PostAPI("admin/zoneWiseRestaurants", {
      zoneId: zoneId,
      businessType: "3", // 2 for stores
    });
    ...
    if (res?.data?.status === "1") {
      ...
      stores.forEach((store) => {
        options.push({ value: store.id, label: store.businessName });
      });
      setStoreList(options);
    } else {
      setStoreList([{ value: null, label: "All Stores" }]);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchStores(filterData.zone?.value || null);
  }, [filterData.zone]);

  useEffect(() => {
    getData();
  }, [filterData?.zone?.value, filterData?.store?.value, filterData?.to]);
            ...
            <div className="flex gap-x-3 items-center">
              ...
              <div>
                <Select
                  ...
                  options={storeList}
                  onChange={(e) => {
                    setFilterData({ ...filterData, store: e });
                  }}
                  value={filterData?.store}
                />
              </div>
              <div>
                <Select

...
const Rejected = () => {
  ...
  const datas = [];
  const csv = [];
  rejectedOrderData()?.map((values, index) => {
    csv.push({
      sn: index + 1,
      id: values?.id,
      orderNum: values?.orderNum,
      restaurantName: values?.restaurant?.name,
      customerInfo: `${values?.user?.firstName} ${values?.user?.lastName}`,
    ...
    return datas.push({
      ...
      deliveryFees:
        values?.restaurant?.zoneRestaurant?.zone?.zoneDetail?.currencyUnit
          ?.symbol +
        " " +
        (values?.orderCharge?.deliveryFees || "0.00"),
      serviceFees: values?.restaurant?.zoneRestaurant?.zone?.zoneDetail
        ?.currencyUnit?.symbol
        ? values?.restaurant?.zoneRestaurant?.zone?.zoneDetail?.currencyUnit
            ?.symbol +
          " " +
          (values?.orderCharge?.serviceCharges || "0.00")
        : "0.00",
      packingFees:
        values?.restaurant?.zoneRestaurant?.zone?.zoneDetail?.currencyUnit
          ?.symbol +

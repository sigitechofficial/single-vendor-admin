...
export default function EditRestaurants() {
  ...
  let { resId } = useParams();
  const { data, reFetch } = GetAPI(`admin/getmetadatarestaurant/${resId}`);
  const { data: restEarnings, reFetch: restEarningsRefetch } = GetAPI(
    `admin/restaurantEarnings/${resId}`
  );
  ...
  const isMarkerInsidePolygon = (markerPosition, polygon) => {
    const google = window.google;
    const markerLatLng = new google.maps.LatLng(
      markerPosition.lat,
      markerPosition.lng
    );
    const polygonBounds = new google.maps.Polygon({
      paths: polygon,
    });
